        ...
        <div class="card border-0 shadow-5sm rounded col-6">
            <div class="card-body">
                <form action="{{ route('categories.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <h1 class="d-flex justify-content-center">Tambah Category</h1>
                    <div class="form-group">
                        <label class="font-weigt-bold">Nama</label>
                        <input type="text" class="form-control @error('nama') is-invalid 
                        @enderror" name="nama" value="{{ old('nama',$data->nama)}}">
                        @error('nama')
                    ...
                    <div class="form-group">
                        ...
                        @enderror" name="slung" value="{{ old('slung',$data->slung)}}">
                        @error('slung')
                            <div class="alert alert-danger mt-2">
                                {{ $message}}
                            </div>
